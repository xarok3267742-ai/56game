...
For the final upload-day owner preflight, use `./tools/run_upload_day_preflight.py`. It runs the hosted-privacy final local gate, regenerates the internal store asset review sheet, writes and verifies the generated owner handoff ZIP, reprints upload/Play Console packets and rechecks publication readiness. Use `--managed-api36-connected` to route through the project-owned API 36 AVD helper, or `--release-tag <release-tag>` after pushing a tagged release to include remote verification.
