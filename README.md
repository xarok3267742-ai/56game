...
Последняя локальная проверка на 14 июня 2026: `./tools/run_final_local_gate.py --include-connected --connected-serial emulator-5560 --include-hosted-privacy` прошёл с `final_local_gate_ok`; connected run выполнил 10/10 тестов на `Medium_Phone_API_36(AVD) - 16`, `./tools/verify_remote_release.py --tag v1.0.0-rc72` прошёл с `remote_release_ok`, and `./tools/run_upload_day_preflight.py --release-tag v1.0.0-rc72` прошёл с `upload_day_preflight_ok`. Gate включает Google Play sources, product decision, performance notes, assets, privacy/safety handoff, signing report, metadata, release cleanliness, fresh debug/release artifact evidence, mandatory fresh connected XML evidence and hosted privacy URL validation; TalkBack exploratory pass выполнен отдельно на API 36 Play Store AVD, artifacts сохранены в `docs/qa_artifacts`.
