...
Локально подготовленный release candidate не равен опубликованному Google Play продукту. Перед публикацией ещё нужно заполнить рабочий Play Console support contact, разместить `play_store/privacy_policy_ru.html` на публичном HTTPS URL, проверить URL командой `./tools/check_privacy_policy_url.py --url <https-url>`, сделать backup keystore/credentials after `./tools/check_signing_backup_inputs.py` returns `signing_backup_input_ok`, заполнить Play Console forms and run required testing tracks for the publisher account type.
