...
- After pushing the release handoff to GitHub, run `./tools/verify_remote_release.py --tag <release-tag>` and require `remote_release_ok`; it verifies `origin/main`, the remote release tag, the remote signed AAB checksum, remote forbidden-path hygiene, `origin/gh-pages` privacy-policy presence and the recorded hosted privacy URL.
