...
After Play-generated artifact review, the icon line must be based on helper output `store icon pixel matches: ...` and `application icon linked store icon: ...`, not only a visual/manual size check.
