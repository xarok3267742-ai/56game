...
- Generated artifacts must not contain debug package ids, androidTest/JUnit/Espresso/test leakage, forbidden permissions, icon pixels that differ from `play_store/icon/play_icon_512.png`, application/round icon references that are not linked to that matching PNG, or native `.so` files below 16 KB page-size alignment.
