...
- После push в GitHub `./tools/verify_remote_release.py --tag <release-tag>` возвращает `remote_release_ok`, подтверждая `origin/main`, remote release tag, remote AAB checksum, отсутствие signing/install artifacts в remote tree, наличие privacy HTML на `origin/gh-pages` and recorded hosted privacy URL validity.
