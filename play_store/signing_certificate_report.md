...
- Local `local.properties`, `keystore.properties`, `private/signing/*.p12`, common signing-key extensions (`*.jks`, `*.keystore`, `*.pem`, `*.pk8`, `*.key`) and Android upload/install artifact extensions (`*.apk`, `*.aab`, `*.apks`, `*.idsig`) are ignored case-insensitively by `.gitignore`; `tools/verify_release.py` verifies representative lower/upper-case paths with `git check-ignore`, fails if forbidden sensitive/install paths are tracked in Git, and the active local signing files currently have owner-only filesystem permissions.
