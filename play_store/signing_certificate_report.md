...
- Local `local.properties`, `keystore.properties`, `private/signing/*.p12` and case-insensitive common signing-key extensions (`*.jks`, `*.keystore`, `*.pem`, `*.pk8`, `*.key`) are ignored by `.gitignore`; the active local signing files currently have owner-only filesystem permissions.
